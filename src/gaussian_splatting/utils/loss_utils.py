...
def l1_loss_appearance(image: torch.Tensor,
                       gt_image: torch.Tensor,
                       gaussian_model: GaussianModelRaDeGS,
                       view_idx: int,
                       return_transformed_image: bool = False):
    appearance_embedding = gaussian_model.get_apperance_embedding(view_idx)
    ...
    crop_image_down = \
        torch.cat([crop_image_down, appearance_embedding[None].repeat(H // 32, W // 32, 1).permute(2, 0, 1)], dim=0)[
            None]
    mapping_image = gaussian_model.appearance_network(crop_image_down)
    transformed_image = mapping_image * crop_image
    if not return_transformed_image:
        return l1_loss(transformed_image, crop_gt_image)
    else:
        transformed_image = \
            torch.nn.functional.interpolate(transformed_image, size=(origH, origW), mode="bilinear",
